<div x-data="{ openDelete: false }">
    ...
    <template x-teleport="body">
        <div x-show="openDelete" x-cloak x-transition:enter="transition ease-out duration-50"
            ...
            <div class="relative z-[10000] w-full max-w-sm bg-white rounded-2xl shadow-2xl border border-slate-100 p-6 space-y-4">
            <div class="flex items-start space-x-3">
                <div class="w-10 h-10 rounded-full text-red-600 flex items-center justify-center font-black text-lg">
                    <x-lucide-circle-alert />
                </div>
                <div>
                    <h4 class="text-lg font-bold text-slate-800">Delete Size Group?</h4>
                    <p class="text-sm text-slate-500">This will also delete all size values in this group. This action cannot be undone.</p>
                </div>
            </div>
                @if ($errorMessage)
            ...
            <div class="flex items-center justify-end gap-3 pt-2">
                <button type="button" @click="openDelete = false"
                    class="text-xs font-bold text-slate-500 uppercase tracking-widest hover:text-slate-700 transition-colors">Cancel</button>
                    @if (!$errorMessage)
