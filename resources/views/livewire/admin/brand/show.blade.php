            ...
            <div class="px-8 py-10 bg-slate-50/50 border-b border-slate-100">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div class="flex items-center space-x-5">
                        ...
                        <div>
                            <h1 class="text-3xl font-black tracking-tight text-slate-900 uppercase italic">Detail Brand</h1>
                            <p class="text-slate-500 font-bold mt-1 uppercase text-xs tracking-widest">Viewing record ID: #{{ $brand->id }}</p>
                        </div>
                    </div>

                    <div>
                        @if ($brand->is_trusted)
                            <span class="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest border border-emerald-100 shadow-sm">
                                <x-lucide-badge-check class="w-4 h-4" />
                                Trusted Brand
                            </span>
                        @else
                            <span class="inline-flex items-center gap-2 bg-slate-100 text-slate-600 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest border border-slate-200 shadow-sm">
            ...
            <div class="p-8 md:p-12">
                <div class="space-y-12">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-4">
                            <label class="block text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Official Logo</label>
                            <div class="aspect-square bg-slate-50 rounded-[2rem] border border-slate-100 flex items-center justify-center p-8 shadow-sm group hover:bg-white transition-all duration-500">
                                @if ($brand->logo)
                                ...
                                @else
                                    <div class="text-center">
                                        ...
                                        <p class="text-[10px] font-bold text-slate-300 uppercase tracking-widest">No Logo</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <label class="block text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Brand Thumbnail</label>
                            <div class="aspect-square bg-slate-50 rounded-[2rem] border border-slate-100 overflow-hidden shadow-sm group">
                                @if ($brand->image)
                        ...
                        <div class="md:col-span-2 space-y-4">
                            <label class="block text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Wide Banner Image</label>
                            <div class="relative h-64 bg-slate-50 rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-sm">
                                @if ($brand->wide_image)
                                ...
                                @else
                                    <div class="flex flex-col items-center justify-center h-full">
                                        ...
                                        <p class="text-[10px] font-bold text-slate-300 uppercase tracking-widest">No Banner Image</p>
                                    </div>
                                @endif
                            </div>
                        </div>
