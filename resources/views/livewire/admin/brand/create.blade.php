            ...
            <div class="px-8 py-10 bg-slate-50/50 border-b border-slate-100">
                <div class="flex items-center space-x-5">
                    ...
                    <div>
                        ...
                        <p class="text-slate-500 font-medium mt-1">Daftarkan mitra brand baru ke dalam sistem katalog.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-8 md:p-12">
                <form wire:submit.prevent="save" class="space-y-8">
                    ...
                    <div class="pt-2">
                        <label class="relative flex items-center p-5 bg-slate-50 rounded-3xl cursor-pointer hover:bg-slate-100 transition-all border border-transparent hover:border-emerald-100 group">
                            ...
                            <div class="ml-4">
                                ...
                                <span class="text-xs text-slate-400 font-medium italic">Berikan lencana verifikasi pada brand ini.</span>
                            </div>
                        </label>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-3">
                            <label class="block text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Brand Logo (1:1)</label>
                            <div class="relative group">
                                <input wire:model="logo" type="file" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                                <div class="border-2 border-dashed border-slate-200 rounded-[2rem] p-8 flex flex-col items-center justify-center bg-slate-50/50 group-hover:bg-emerald-50/30 group-hover:border-emerald-200 transition-all">
                                    @if ($logo)
                                        <img src="{{ $logo->temporaryUrl() }}" class="w-24 h-24 object-contain rounded-xl shadow-md">
                                    @else
                                        <x-lucide-box class="w-8 h-8 text-slate-300 mb-2" />
                                        <span class="text-[10px] font-black text-slate-400 uppercase italic">Pilih Logo</span>
                                    @endif
                                </div>
                            </div>
                            @error('logo') <span class="text-red-500 text-[10px] font-black uppercase tracking-wider block">{{ $message }}</span> @enderror
                        </div>

                        <div class="space-y-3">
                            <label class="block text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Thumbnail Image</label>
                            <div class="relative group">
                                <input wire:model="image" type="file" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                                <div class="border-2 border-dashed border-slate-200 rounded-[2rem] p-8 flex flex-col items-center justify-center bg-slate-50/50 group-hover:bg-emerald-50/30 group-hover:border-emerald-200 transition-all">
                                    @if ($image)
                                        <img src="{{ $image->temporaryUrl() }}" class="w-24 h-24 object-cover rounded-xl shadow-md">
                                    @else
                                        <x-lucide-image class="w-8 h-8 text-slate-300 mb-2" />
                                        <span class="text-[10px] font-black text-slate-400 uppercase italic">Pilih Thumbnail</span>
                                    @endif
                                </div>
                            </div>
                            @error('image') <span class="text-red-500 text-[10px] font-black uppercase tracking-wider block">{{ $message }}</span> @enderror
                        </div>

                        <div class="md:col-span-2 space-y-3">
                            <label class="block text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Wide Banner (Hero)</label>
                            <div class="relative group">
                                <input wire:model="wide_image" type="file" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                                <div class="border-2 border-dashed border-slate-200 rounded-[2rem] p-10 flex flex-col items-center justify-center bg-slate-50/50 group-hover:bg-emerald-50/30 group-hover:border-emerald-200 transition-all">
                                    @if ($wide_image)
                                        <img src="{{ $wide_image->temporaryUrl() }}" class="w-full h-32 object-cover rounded-[1.5rem] shadow-sm">
                                    @else
                                        <x-lucide-layout-panel-top class="w-10 h-10 text-slate-300 mb-2" />
                                        <span class="text-[10px] font-black text-slate-400 uppercase italic text-center">Pilih Banner Utama<br>(Wide Format)</span>
                                    @endif
                                </div>
                            </div>
                            @error('wide_image') <span class="text-red-500 text-[10px] font-black uppercase tracking-wider block">{{ $message }}</span> @enderror
                        </div>
                    </div>

                    <div wire:loading class="w-full">
                        <div class="flex items-center justify-center p-4 bg-emerald-50 rounded-2xl animate-pulse border border-emerald-100">
                            <span class="text-emerald-600 text-[10px] font-black uppercase tracking-widest">Aset sedang diproses...</span>
                        </div>
                    </div>
