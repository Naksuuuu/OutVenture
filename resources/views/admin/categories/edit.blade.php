    ...
    <div class="max-w-xl bg-white rounded-3xl p-8 border border-gray-100 shadow-sm mx-auto">
        ...
        <form action="{{ route('categories.update', $category->id) }}" method="POST">
            ...
            <div class="flex items-center gap-4 mt-10">
                <button type="submit" class="flex-1 bg-emerald-500 text-white py-4 rounded-2xl font-bold hover:bg-emerald-600 transition shadow-lg shadow-emerald-100">
                    Update Category
                </button>
                <a href="{{ route('categories.index') }}" class="px-8 py-4 text-gray-400 font-bold hover:text-gray-600 transition">
                    Cancel
                </a>
            </div>
        </form>
